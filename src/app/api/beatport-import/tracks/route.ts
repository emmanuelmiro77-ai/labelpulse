...
import { runTrackImport } from "@/services/beatport-import/track-import";

/**
 * API /api/beatport-import/tracks
 *
 * BP-003 — Beatport Track Import Service
 *
 * Isolated endpoint that accepts a scraper JSON (or a file upload) and
 * imports only the `tracks` array into the Supabase global tracks row.
 *
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const report = await runTrackImport(scraperJson, supabaseUrl, serviceKey, { dryRun });

    return NextResponse.json({ ok: true, report });
  } catch (err: any) {
    console.error("[/api/beatport-import/tracks] error:", err);
    return NextResponse.json({ error: err.message }, { status: 500 });
  }
}

/**
 * GET /api/beatport-import/tracks
 *
...
export async function GET() {
  ...
  return NextResponse.json({
    service: "beatport-import/tracks",
    status: "ready",
    description: "POST a scraper JSON (inline or file upload) to import tracks. Use ?dryRun=true to preview.",
  });
}
